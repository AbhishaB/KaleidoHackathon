...
class SimpleStorage {
    ...
    set(stringValue, stringValue1) {
        // Change json string to int value
        let value = parseInt(stringValue1);
        ...
        return this.node1.then((web3) => {
            ...
            return web3.eth.getAccounts().then(accounts => {
                ...
                return this.contractInterface.methods.set(stringValue, value).estimateGas({from: defaultAccount}).then((gasAmount) => {
                    console.log("Sending the transaction to set a value");
                    // Send the transaction with the estimated gas
                    return this.contractInterface.methods.set(stringValue, value).send({from: defaultAccount, gas: gasAmount});
                });
            });
        });
    }
...
//		let value1 = parseInt(stringValue1);
...
//                console.log("Estimating the gas required to set a value");
                ...
                // In Kaleido's networks, the gas price is set to 0; so while you must specify an amount of gas to use,
                // it doesn't actually cost anything.
//                return this.contractInterface.methods.set(value, value1).estimateGas({from: defaultAccount}).then((gasAmount) => {
//                    console.log("Sending the transaction to set a value");
                    // Send the transaction with the estimated gas
//                    return this.contractInterface.methods.set(value, value1).send({from: defaultAccount, gas: gasAmount});
//                });
//            });
//        });
//    }
