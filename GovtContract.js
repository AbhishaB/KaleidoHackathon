...
// This serves as a js wrapper class for interacting with the GovtContract.sol smart contract
class GovtContract {
    ...
    set(stringValue1, stringValue2, stringValue3, stringValue4, stringValue5, stringValue6) {
        // Change json string to int value
        let intValue1 = parseInt(stringValue1);
		let intValue2 = parseInt(stringValue2);
		let intValue3 = parseInt(stringValue6);
        ...
        return this.node1.then((web3) => {
            ...
            return web3.eth.getAccounts().then(accounts => {
                ...
                return this.contractInterface.methods.set(intValue1, intValue2, stringValue3, stringValue4, stringValue5, intValue3).estimateGas({from: defaultAccount}).then((gasAmount) => {
                    console.log("Sending the transaction to set a value");
                    // Send the transaction with the estimated gas
                    return this.contractInterface.methods.set(intValue1, intValue2, stringValue3, stringValue4, stringValue5, intValue3).send({from: defaultAccount, gas: gasAmount});
                });
            });
        });
    }
